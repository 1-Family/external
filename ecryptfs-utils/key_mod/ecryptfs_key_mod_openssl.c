...
#define OPENSSL_DATA_PASSPHRASE 1
#define OPENSSL_DATA_PATH 2

typedef enum {ENCRYPT, DECRYPT} action_t;
				      ...
				      struct openssl_data *openssl_data)
{
	...
	if (!openssl_data->path || !openssl_data->passphrase) {
		rc = -EINVAL;
		ALOGE("openssl_data internal structure not "
		       "properly filled in\n");
		goto out;
	}
...
ecryptfs_openssl_write_key_to_file(RSA *rsa, char *filename, char *passphrase)
{
	...
	if (rc) {
		ALOGW("%s: Error attempting to mkdir [%s]; "
		       "rc = [%d]\n", __FUNCTION__, openssl_dir, rc);
	}
	if ((out = BIO_new(BIO_s_file())) == NULL) {
		ALOGE("Unable to create BIO for output\n");
		rc= -EIO;
		goto out;
	}
	if (BIO_write_filename(out, filename) <= 0) {
		ALOGE("Failed to open file for reading\n");
		rc = -EIO;
		goto out_free_bio;
	}
	if (!PEM_write_bio_RSAPrivateKey(out, rsa, enc, NULL, 0, NULL,
					 (void *)passphrase)) {
		ALOGE("Failed to write key to file\n");
		rc = -EIO;
		goto out_free_bio;
	}
out_free_bio:
...
static int ecryptfs_openssl_read_key(RSA **rsa, unsigned char *blob)
{
	...
	if (BIO_read_filename(in, openssl_data->path) <= 0) {
		ALOGE("Unable to read filename [%s]\n",
		       openssl_data->path);
		rc = -EIO;
		goto out;
	}
	if ((*rsa = PEM_read_bio_RSAPrivateKey(in, NULL, NULL,
	    ...
	    == NULL) {
		ALOGE(		       "%s: Unable to read private key from file [%s]\n",
...
int ecryptfs_openssl_get_key_sig(unsigned char *sig, unsigned char *blob)
{
	...
	if ((rc = ecryptfs_openssl_read_key(&rsa, blob))) {
		ALOGE("Error attempting to read RSA key from file;"
		       " rc = [%d]\n", rc);
		goto out;
	}
	if ((rc = ecryptfs_openssl_generate_signature((char *)sig, rsa))) {
		ALOGE("%s: Error attempting to generate key "
		       "signature; rc = [%d]\n", __FUNCTION__, rc);
		goto out_free_rsa;
	}
out_free_rsa:
...
static int ecryptfs_openssl_generate_key(struct openssl_data *openssl_data)
{
	...
	BN_set_word(e, 65537);
	if ((void *)RSA_generate_key_ex(rsa, 1024, e, NULL) == NULL) {
		ALOGE("Error generating new RSA key\n");
		rc = -ENOMEM;
		goto out_free_rsa;
	}
	if ((rc = ecryptfs_openssl_write_key_to_file(
		     rsa, openssl_data->path, openssl_data->passphrase))) {
		ALOGE("Error writing key to file; rc = [%d]\n", rc);
		rc = -EIO;
		goto out_free_rsa;
	}
out_free_rsa:
...
int get_key_from_server(pid_t pid, uid_t user, action_t action, const unsigned char * from_key, size_t from_key_size, unsigned char * to_key, size_t expected_to_size,ssize_t * recv_bytes) {
	int rc = 0;
	...
	if (rc >= 0) {
		...
		if (fd >= 0) {
			...
			if (rc >= 0) {
				...
				if (rc >= 0) {
					//ALOGD("The remote socket uid is <%d>\n", credentials.uid);
					if (credentials.uid == user) {
						...
						if (message) {
							...
							rc = -1;
							ALOGE("Error allocating memory for message\n");
						}
					} else {
						ALOGE("Error - The server is run by a wrong uid <%d>\n", credentials.uid);
						rc = -1;
					}
				} else {
					ALOGE("Error authenticating server, <%s>\n", strerror(errno));
				}
			} else {
				ALOGE("%s", strerror(errno));
			}
...
static int get_encrypted_key_from_server(pid_t pid, uid_t user, const unsigned char * from_key, size_t from_key_size, unsigned char * to_key,size_t expected_to_size, ssize_t * recv_bytes) {
	return get_key_from_server(pid, user, ENCRYPT, from_key, from_key_size, to_key, expected_to_size, recv_bytes);
}

static int get_decrypted_key_from_server(pid_t pid, uid_t user, const unsigned char * from_key, size_t from_key_size, unsigned char * to_key, size_t expected_to_size, ssize_t * recv_bytes) {
	return get_key_from_server(pid, user, DECRYPT, from_key, from_key_size, to_key, expected_to_size, recv_bytes);
}
...
static int ecryptfs_openssl_encrypt(char *to, size_t *to_size, char *from,
				    ...
				    int blob_type, uid_t euid, pid_t pid)
{
	RSA *rsa = NULL;
	int rc;
	...
	(*to_size) = ECRYPTFS_MAX_ENCRYPTED_KEY_BYTES;
	if (to) {
		ssize_t nread = 0;
		rc = get_encrypted_key_from_server(pid, euid, (unsigned char *)from, from_size, (unsigned char *)to, ECRYPTFS_MAX_ENCRYPTED_KEY_BYTES ,&nread);
		if (rc == 0) {
			//ALOGD("ecryptfs_openssl_encrypt - sent <%s>\n", from);
			//ALOGD("ecryptfs_openssl_encrypt - received <%s>\n", to);
		} else {
			ALOGE("ecryptfs_openssl_encrypt - get_key_from_server failed!\n");
		}
		//ALOGD("ecryptfs_openssl_encrypt - nread is now <%u>\n", nread);
		(*to_size) = (size_t)nread;
	}
out_free_rsa:
	RSA_free(rsa);
out:
				    ...
				    int blob_type, uid_t euid, pid_t pid)
{
	...
	(*to_size) = ECRYPTFS_MAX_KEY_BYTES;
	if (to) {
		ssize_t nread = 0;
		rc = get_decrypted_key_from_server(pid, euid, (unsigned char *)from, from_size, (unsigned char *)to, ECRYPTFS_MAX_KEY_BYTES, &nread);
		if (rc == 0) {
			(*to_size) = (size_t)nread;
			//ALOGD("ecryptfs_openssl_decrypt - sent <%s>\n", from);
			//ALOGD("ecryptfs_openssl_decrypt - received <%s>\n", to);
		} else {
			ALOGE("ecryptfs_openssl_decrypt - get_key_from_server failed!\n");
		}
	}
out_free_rsa:
	RSA_free(rsa);
...
int ecryptfs_openssl_init_from_param_vals(struct openssl_data * openssl_data, struct key_mod_param_val * param_vals, uint32_t num_param_vals) {
	int rc = 0;
	struct key_mod_param_val * walker = param_vals;
	uint32_t i = 0;
	for (i=0; i< num_param_vals;i++) {
		switch(walker->id) {
			case OPENSSL_DATA_PATH:
				if (asprintf(&openssl_data->path, "%s", walker->val) == -1) {
						rc = -ENOMEM;
						goto out;
				}
				break;
			case OPENSSL_DATA_PASSPHRASE:
				if (asprintf(&openssl_data->passphrase, "%s", walker->val) == -1) {
						rc = -ENOMEM;
						goto out;
				}
				break;
				     ...
				     uint32_t num_param_vals)
{
	...
	if ((rc = ecryptfs_openssl_init_from_param_vals(&openssl_data,
							param_vals,
							num_param_vals))) {
		ALOGE("Error parsing parameter values; rc = [%d]\n", rc);
		goto out;
	}
	if (blob == NULL) {
		if ((rc = ecryptfs_openssl_serialize(NULL, blob_size, &openssl_data))) {
			ALOGE("Error serializing openssl; rc = [%d]\n", rc);
			goto out;
		}
		goto out;
	}
...
out:
	if (openssl_data.passphrase) {
		free(openssl_data.passphrase);
		openssl_data.passphrase = NULL;

	}
	if (openssl_data.path) {
		free(openssl_data.path);
		openssl_data.path = NULL;
	}
	return rc;
}
			     ...
			     struct val_node **mnt_params)
{
	...
	if (blob_size == 0) {
		ALOGE("Error serializing openssl\n");
		rc = MOUNT_ERROR;
		goto out;
	}
	...
	if (subgraph_ctx->key_mod->blob_size != blob_size) {
		ALOGE("%s: Internal error\n", __FUNCTION__);
		exit(1);
	}
	if ((rc = ecryptfs_add_key_module_key_to_keyring(
		     sig, subgraph_ctx->key_mod)) < 0) {
		ALOGE("Error attempting to add key to keyring for "
		       "key module [%s]; rc = [%d]\n",
			 ...
			 struct val_node **mnt_params, void **foo)
{
	...
	if ((rc = ecryptfs_openssl_process_key(subgraph_ctx, mnt_params))) {
		ALOGE("Error processing OpenSSL key; rc = [%d]", rc);
		goto out;
	}
			      ...
			      struct val_node **mnt_params, void **foo)
{
	...
	if (fd == -1) {
		ALOGE("%s: Error attempting to open file\n",
		       __FUNCTION__);
		rc = MOUNT_ERROR;
		goto out;
	}
	...
	close(fd);
	if (rc) {
		ALOGE("%s: Error attempting to parse options out "
		       "of file\n", __FUNCTION__);
		goto out;
	}
	...
	if (!walker) {
		ALOGE("%s: No openssl_passwd option found in file\n",
		       __FUNCTION__);
		rc = MOUNT_ERROR;
		goto out;
	}
	...
	struct val_node **mnt_params, void **foo)
{
	...
	if ((rc = ecryptfs_openssl_generate_key(&subgraph_ctx->openssl_data))) {
		ALOGE("%s: Error generating key to file [%s]; "
		       "rc = [%d]\n", __FUNCTION__,
			    ...
			    struct val_node **mnt_params, void **foo)
{
	...
	if ((rc = ecryptfs_find_key_mod(&subgraph_ctx->key_mod, ctx,
					param_node->val))) {
		ALOGE("%s: Cannot find key_mod for param_node with "
