...
int ecryptfs_get_loaded_ciphers(struct ecryptfs_cipher_elem *cipher_list_head)
{
	...
	if (rc) {
		ALOGW("Error attempting to find proc mount "
	...
	if (!(crypto_file = fopen(crypto_full_path, "r"))) {
		ALOGW("%s: Failed to open [%s] for reading; This "
...
int ecryptfs_get_module_ciphers(struct cipher_descriptor *cd_head)
{
	...
	rc = uname(&kern_info);
	if (rc) {
		ALOGE("%s: uname returned [%d]\n", __FUNCTION__, rc);
		goto out;
	}
	rc = asprintf(&kernel_crypto_dir, "/lib/modules/%s/kernel/crypto",
		      kern_info.release);
	if (rc == -1) {
		ALOGE("%s: Error building kernel location string\n",
		       __FUNCTION__);
		rc = -ENOMEM;
		goto out;
	}
	...
	if (!dir) {
		ALOGW("%s: opendir error on [%s]. Cannot get a "
