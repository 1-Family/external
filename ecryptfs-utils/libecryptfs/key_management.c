					  ...
					  char *salt, char *passphrase)
{
	...
	*auth_tok = NULL;
	...
	if (rc) {
		ALOGE("Error generating passphrase signature; "
		       "rc = [%d]\n", rc);
		rc = (rc < 0) ? rc : rc * -1;
		goto out;
	}
	*auth_tok = malloc(sizeof(struct ecryptfs_auth_tok));
	if (!*auth_tok) {
		ALOGE("Unable to allocate memory for auth_tok\n");
		rc = -ENOMEM;
		goto out;
	}
	rc = generate_payload(*auth_tok, auth_tok_sig, salt, fekek);
	if (rc) {
		ALOGE("Error generating payload for auth tok key; "
		       "rc = [%d]\n", rc);
		rc = (rc < 0) ? rc : rc * -1;
		goto out;
	}
...
binary_data ecryptfs_passphrase_blob(char *salt, char *passphrase)
{
	...
	if (rc) {
		ALOGE("%s: Error attempting to generate passphrase "
...
int ecryptfs_remove_auth_tok_from_keyring(char *auth_tok_sig)
{
	...
	if (rc < 0) {
		rc = errno;
		ALOGE("Failed to find key with sig [%s]: %m\n",
		       auth_tok_sig);
		goto out;
	}
	...
	if (rc < 0) {
		rc = errno;
		ALOGE("Failed to unlink key with sig [%s]: %s\n",
				     ...
				     char *auth_tok_sig)
{
	...
	if (rc != -1) { /* we already have this key in keyring; we're done */
		...
		ALOGE("keyctl_search failed: %m errno=[%d]\n",
		       errnum);
	...
	if (rc == -1) {
		rc = -errno;
		ALOGE("Error adding key with sig [%s]; rc = [%d] "
		       "\"%m\"\n", auth_tok_sig, rc);
		if (rc == -EDQUOT)
			ALOGW("Error adding key to keyring - keyring is full\n");
		goto out;
	}
	rc = 0;
out:
					   ...
					   char *salt)
{
	...
	rc = ecryptfs_generate_passphrase_auth_tok(&auth_tok, auth_tok_sig,
						   fekek, salt, passphrase);
	if (rc) {
		ALOGE("%s: Error attempting to generate the "
	...
	if (rc < 0) {
		ALOGE("%s: Error adding auth tok with sig [%s] to "
...
int ecryptfs_append_sig(char *auth_tok_sig, char *sig_cache_filename)
{
	...
	fd = open(sig_cache_filename, (O_WRONLY | O_CREAT),
		  (S_IRUSR | S_IWUSR));
	if (fd == -1) {
		ALOGE("Open resulted in [%d]; [%m]\n", errno);
		rc = -EIO;
		goto out;
	}
	if (fchown(fd, getuid(), getgid()) == -1) {
		ALOGW("Can't change ownership of sig file; "
				    "errno = [%d]; [%m]\n", errno);
	}
	lseek(fd, 0, SEEK_END);
	...
	if ((size = write(fd, tmp, (ECRYPTFS_SIG_SIZE_HEX + 1))) !=
	    (ECRYPTFS_SIG_SIZE_HEX + 1)) {
		ALOGE("Write of sig resulted in [%zu]; errno = [%d]; "
		       "[%m]\n", size, errno);
		rc = -EIO;
		close(fd);
		goto out;
...
int ecryptfs_validate_keyring(void)
{
	...
	if ((rc_long = keyctl(KEYCTL_LINK, KEY_SPEC_USER_KEYRING,
		...
		ALOGE("Error attempting to link the user session "
				       ...
				       struct ecryptfs_key_mod *key_mod)
{
	...
	if (key_mod->blob == NULL) {
		if ((rc = (key_mod->ops->get_blob)(NULL, &blob_size,
			...
			ALOGE("Error attempting to get blob from "
			       "key module; rc = [%d]\n", rc);
			goto out;
		}
	} else {
	...
	if ((rc = ecryptfs_generate_key_payload(auth_tok, key_mod, auth_tok_sig,
						blob_size))) {
		ALOGE("Error initializing key from module; "
		       "rc = [%d]\n", rc);
		goto out;
	}
	rc = (int)keyctl_search(KEY_SPEC_USER_KEYRING, "user", auth_tok_sig, 0);
	...
	if (rc < 0) {
		rc = -errno;
				printf("Error adding key with sig [%s]; rc ="
		       " [%d]\n", auth_tok_sig, rc);
		ALOGE("Error adding key with sig [%s]; rc ="
		       " [%d]\n", auth_tok_sig, rc);
		if (rc == -EDQUOT)
			ALOGW("Error adding key to keyring - keyring is full\n");
	} else rc = 0;
