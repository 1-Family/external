...
{
	ALOGW("%s: Dummy function substituted for unimplemented "
...
ecryptfs_dummy_encrypt(char *to, size_t *to_size, char *from, size_t from_size,
		       unsigned char *blob, int blob_type, uid_t euid, pid_t pid)
{
	ALOGW("%s: Dummy function substituted for unimplemented "
	       "function in key module\n", __FUNCTION__);
	(*to_size) = 0;
...
int ecryptfs_register_key_modules(struct ecryptfs_ctx* ctx)
{
	...
	while ((ep = readdir(dp))) {
		...
		if (!new_key_mod->ops) {
			ALOGE("Library function get_key_mod_ops() "
		...
		if ((rc = ecryptfs_fill_in_dummy_ops(new_key_mod->ops))) {
			ALOGE("Error attempting to fill in missing  "
		...
		if ((rc = new_key_mod->ops->init(&new_key_mod->alias))) {
			ALOGE("Error initializing key module [%s]; "
			       "rc = [%d]\n", path, rc);
			free(new_key_mod);
			rc = 0;
			goto end_loop;
	...
	while (walker) {
		...
		if ((rc = new_key_mod->ops->init(&new_key_mod->alias))) {
			ALOGE("Error initializing key module in "
			       "array position [%d]\n", i);
			free(new_key_mod);
			rc = 0;
			goto end_loop_2;
