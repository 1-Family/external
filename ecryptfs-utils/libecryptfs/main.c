			      ...
			      char *sig, size_t blob_size)
{
	...
	if (key_mod->blob == NULL) {
		if ((rc = (key_mod->ops->get_blob)
			...
			ALOGE("Call into key module's get_blob "
			      "failed; rc = [%d]\n", rc);
			goto out;
		}
	} else {
	...
	if (blob_size != blob_size_tmp) {
		rc = -EINVAL;
		ALOGE("BUG: blob_size != blob_size_tmp; key module "
	...
	if ((rc = (key_mod->ops->get_key_data)
		...
		ALOGE("Call into key module's get_key_data failed; "
		       "rc = [%d]\n", rc);
		goto out;
	}
	if (key_data_len == 0) {
		if ((rc = (key_mod->ops->get_key_sig)(
			...
			ALOGE("Call into key module's get_key_sig "
			      "failed; rc = [%d]\n", rc);
			goto out;
		}
	} else {
