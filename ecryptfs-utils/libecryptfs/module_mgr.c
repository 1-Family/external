					      ...
					      int min, int max)
{
	...
	while (supported_key_bytes[i].cipher_name) {
		if ((supported_key_bytes[i].key_bytes >= (uint32_t)min) &&
		    (supported_key_bytes[i].key_bytes <= (uint32_t)max) &&
	...
	if (ecryptfs_key_bytes_param_node.num_transitions == 0) {
		ALOGE("Error initializing key_bytes selection: "
			      ...
			      struct val_node **head, void **foo)
{
	...
	if (rc) {
		ALOGE("%s: Error initializing key_bytes param node; "
		       "rc = [%d]\n", __FUNCTION__, rc);
		goto out;
	}
	...
	if (ecryptfs_verbosity)
		ALOGI("%s: Pushing onto stack; opt = [%s]\n",
		       __FUNCTION__, opt);
	rc = stack_push(head, opt);
out:
	return rc;
...
static int init_ecryptfs_cipher_param_node(uint32_t version)
{
	...
	if (rc) {
		ALOGE("%s: Error getting kernel ciphers; rc = [%d]\n",
		       __FUNCTION__, rc);
		goto out;
	}
	rc = ecryptfs_get_module_ciphers(&cd_head);
	if (rc) {
		ALOGE("%s: Error getting module ciphers; rc = [%d]\n",
		       __FUNCTION__, rc);
		goto out;
	}
	if (cd_head.next == NULL) {
		ALOGE("%s: Unable to detect any available kernel "
		       "ciphers; bailing out\n", __FUNCTION__);
		rc = -ENOTSUP;
		goto out;
	}
	...
	if (rc) {
		ALOGE("%s: Error sorting ciphers; rc = [%d]\n",
		       __FUNCTION__, rc);
		goto out;
	}
	...
	while (cd_cursor) {
		...
		if (ecryptfs_cipher_param_node.num_transitions
		    >= MAX_NUM_TRANSITIONS) {
			ALOGW("%s: Exceeded maximum number of "
				    ...
				    int key_module_only)
{
	...
	if (rc) {
		ALOGE("Error attempting to get key module list; "
		       "rc = [%d]\n", rc);
		goto out;
	}
	...
	while (key_mod) {
		...
		if ((rc =
		     key_mod->ops->get_param_subgraph_trans_node(&trans_node,
								 version))) {
			ALOGI("Key module [%s] does not have a "
			...
			if ((rc = ecryptfs_build_linear_subgraph(&trans_node,
								 key_mod))) {
				ALOGW("Error attempting to build "
		...
		if ((rc =
		     add_transition_node_to_param_node(&key_module_select_node,
						       trans_node))) {
			ALOGE("Error attempting to add transition "
		...
		if ((rc = ecryptfs_insert_params_in_subgraph(ad_cursor,
							     trans_node))) {
			ALOGE("Error attempting to insert allowed "
	...
	if (key_module_only == ECRYPTFS_ASK_FOR_ALL_MOUNT_OPTIONS) {
		...
		if (rc) {
			ALOGE("%s: Error attempting to fill in "
			       "decision graph; rc = [%d]\n", __FUNCTION__, rc);
			goto out;
		}
	} else
