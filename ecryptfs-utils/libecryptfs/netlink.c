			  ...
			  uint16_t msg_flags, uint32_t msg_seq)
{
	...
	if (!nlh) {
		ecryptfs_ALOGE("Failed to allocate memory for "
				"netlink header: %m\n");
		rc = -ENOMEM;
		goto out;
	}
	...
	if (rc < 0) {
		rc = -errno;
		ALOGE("Failed to send eCryptfs netlink "
		       "message: %m\n");
		goto out;
	}
out:
...
receive:
	...
	if (rc < 0) {
		rc = -errno;
		ALOGE("Failed to receive netlink header; errno = "
		       "[%d]; errno msg = [%m]\n", errno);
		goto free_out;
	}
	if (flags & MSG_PEEK) {
	...
	if (nladdr_len != sizeof(nladdr)) {
		rc = -EPROTO;
		ALOGE("Received invalid netlink message\n");
		goto free_out;
	}
	if (nladdr.nl_pid) {
		rc = -ENOMSG;
		ALOGW("Received netlink packet from a "
...
int ecryptfs_init_netlink(struct ecryptfs_nl_ctx *nl_ctx)
{
	...
	if (nl_ctx->socket_fd == -1) {
		rc = -errno;
		ALOGE("Failed to create the eCryptfs "
		       "netlink socket: [%m]\n");
		goto out;
	}
	...
	if (rc) {
		rc = -errno;
		ALOGE("Failed to bind the eCryptfs netlink "
		       "socket: %m\n");
		goto out;
	}
	ALOGD("eCryptfs netlink socket was successfully "
	       "initialized\n");
out:
	return rc;
}
...
receive:
	...
	if (rc < 0) {
		ALOGE("Error while receiving eCryptfs netlink "
		       "message; errno = [%d]; errno msg = [%m]\n", errno);
		error_count++;
		if (error_count > ECRYPTFS_NETLINK_ERROR_COUNT_THRESHOLD) {
			ALOGE("Netlink error threshold exceeded "
		...
		ALOGD("Received eCryptfs netlink HELO "
		       "message from the kernel\n");
		error_count = 0;
	} else if (msg_type == ECRYPTFS_MSG_QUIT) {
		ALOGD("Received eCryptfs netlink QUIT "
		       "message from the kernel\n");
		free(emsg);
		rc = 0;
		goto out;
		...
		ALOGD("Received unrecognized netlink "
		       "message type [%d]\n", msg_type);
free_emsg:
	free(emsg);
	goto receive;
