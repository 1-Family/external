...
key_mod_encrypt(char **encrypted_key, size_t *encrypted_key_size,
		...
		char *decrypted_key, size_t decrypted_key_size, uid_t euid, pid_t pid)
{
	struct ecryptfs_key_mod *key_mod;
	int rc;

//	ALOGD("The key size before encryption is <%d>\n", decrypted_key_size);
	if (ecryptfs_find_key_mod(&key_mod, ctx,
				  auth_tok->token.private_key.key_mod_alias)) {
		rc = -EINVAL;
		ALOGE("Failed to locate desired key module\n");
		goto out;
	}
	...
	if ((rc = key_mod->ops->encrypt(NULL, encrypted_key_size, decrypted_key,
					...
					ECRYPTFS_BLOB_TYPE_BLOB, euid, pid))) {
		ALOGE("Error attempting to get encrypted key size "
		       "from key module; rc = [%d]\n", rc);
		goto out;
	}
	if ((*encrypted_key_size) == 0) {
		rc = -EINVAL;
		ALOGE("Encrypted key size reported by key module "
	...
	if (((*encrypted_key) = malloc(*encrypted_key_size)) == NULL) {
		rc = -errno;
		ALOGE("Failed to allocate memory: [%m]\n");
		goto out;
	}
	if ((rc = key_mod->ops->encrypt((*encrypted_key), encrypted_key_size,
					...
					auth_tok->token.private_key.data,
					ECRYPTFS_BLOB_TYPE_BLOB, euid, pid))) {
		ALOGE("Failed to encrypt key; rc = [%d]\n", rc);
		goto out;
	}
//	ALOGD("The key size after encryption is <%d>\n", (*encrypted_key_size));
...
key_mod_decrypt(char **decrypted_key, size_t *decrypted_key_size,
		struct ecryptfs_ctx *ctx, struct ecryptfs_auth_tok *auth_tok,
		char *encrypted_key, size_t encrypted_key_size, uid_t euid, pid_t pid)
{
	struct ecryptfs_key_mod *key_mod;
	int rc;

//	ALOGE("The key size before decryption is <%d>\n", encrypted_key_size);
	...
	if ((rc = key_mod->ops->decrypt(NULL, decrypted_key_size,
					...
					auth_tok->token.private_key.data,
					ECRYPTFS_BLOB_TYPE_BLOB, euid, pid))) {
		ALOGE("Failed to get size for decrypted key\n");
		goto out;
	}
	if ((*decrypted_key_size) == 0) {
		rc = -EINVAL;
		ALOGE("Decrypted key size reported by key module "
	...
	if ((rc = key_mod->ops->decrypt(*decrypted_key, decrypted_key_size,
					...
					auth_tok->token.private_key.data,
					ECRYPTFS_BLOB_TYPE_BLOB, euid, pid))) {
		ALOGE("Failed to decrypt key\n");
		goto out;
	}
//	ALOGD("The key size after decryption is <%d>\n", (*decrypted_key_size));
		 ...
		 struct ecryptfs_message **reply)
{
	...
	unsigned char *euid_str  = NULL;
	unsigned char *pid_str   = NULL;
	...
	i += length_size;
	euid_str = malloc(data_size + 1);
	if (!euid_str) {
		rc = -errno;
		ALOGE("Failed to allocate memory: %m\n");
		goto write_failure;
	}
	memcpy(euid_str, &emsg->data[i], data_size);
	euid_str[data_size] = '\0';
	i += data_size;
	...
	i += length_size;
	pid_str = malloc(data_size + 1);
	if (!pid_str) {
		rc = -errno;
		ALOGE("Failed to allocate memory: %m\n");
		goto write_failure;
	}
	memcpy(pid_str, &emsg->data[i], data_size);
	pid_str[data_size] = '\0';
	i += data_size;
	...
	if (key_sub < 0) {
		ALOGE("Could not find key with signature: "
		       "[%s]\n", signature);
		rc = -EINVAL;
		goto write_failure;
	}
	...
	uid_t euid = strtoul((char *)euid_str,NULL,10);
	pid_t pid = strtoul((char *)pid_str,NULL,10);

	switch (packet_type) {
	case ECRYPTFS_TAG_64_PACKET:
		if ((rc = key_mod_decrypt(&key_out, &key_out_size, ctx,
					  auth_tok, key, key_size, euid, pid))) {
			ALOGE("Failed to decrypt key; rc = [%d]\n",
			       rc);
		...
		if ((rc = write_tag_65_packet((unsigned char *)key_out,
					      key_out_size, reply))) {
			ALOGE("Failed to write decrypted "
	...
	case ECRYPTFS_TAG_66_PACKET:
		rc = key_mod_encrypt(&key_out, &key_out_size, ctx, auth_tok,
				     key, key_size, euid, pid);
		if (rc) {
			ALOGE("Failed to encrypt public "
			       "key\n");
			goto write_failure;
		}
		rc = write_tag_67_packet(key_out, key_out_size, reply);
		if (rc) {
			ALOGE("Failed to write encrypted "
			       "key to tag 67 packet\n");
			goto write_failure;
		}
		break;
	default:
		ALOGE("Unrecognized packet type: [%d]\n",
		       packet_type);
		rc = -EINVAL;
		break;
	}
