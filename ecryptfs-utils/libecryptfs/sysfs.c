...
{
	int rc = 0;
	const char * sysfs_mnt = "/sys"; //ido - Changing this method to retrieve /sys because this is the right Android sysfs path 
	*mnt_size = strlen(sysfs_mnt);
	if (mnt) {
		memcpy(mnt, sysfs_mnt, *mnt_size);
	}
	return rc;
}
