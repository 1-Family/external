...
int ecryptfs_mount(char *source, char *target, char *opts)
{
	...
	if (!source) {
		rc = -EINVAL;
		ALOGE("Invalid source directory\n");
		goto out;
	}

	if (!target) {
 		rc = -EINVAL;
 		ALOGE("Invalid target directory\n");
 		goto out;
	}
	...
	if (!fullpath_source) {
		rc = -errno;
		ALOGE("could not resolve full path for source %s [%d]",
			source, -errno);
		goto out;
	}
	...
	if (!fullpath_target) {
		rc = -errno;
		ALOGE("could not resolve full path for target %s [%d]",
			target, -errno);
		goto out;
	}
	...
	pid = fork();
	if (pid == -1) {
		ALOGE("Could not fork process to mount eCryptfs: [%d]\n", -errno);
		rc = -errno;
	} else if (pid == 0) {
 		execl("/system/bin/mount", "mount", "-t", "ecryptfs", "-o", opts, fullpath_source, fullpath_target, NULL);
 		//execl("/system/bin/mount", "mount", "-i", "--no-canonicalize", "-t", "ecryptfs", fullpath_source, fullpath_target, "-o", opts, NULL);

		/* error message shown in console to let users know what was wrong */
		/* i.e. /bin/mount does not exist */
		perror("Failed to execute /system/bin/mount command");
		exit(errno);
	} else {
		pid_child = waitpid(pid, &status, 0);
		if (pid_child == -1) {
			ALOGE("Failed waiting for /bin/mount process: [%d]\n", -errno);
			rc = -errno;
			goto out;
		}
		...
		if (rc) {
			ALOGE("Failed to perform eCryptfs mount: [%d]\n", rc);
			if (-EPIPE == rc) {
				rc = -EPERM;
			}
		}
